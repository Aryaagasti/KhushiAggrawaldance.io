...
const Contact = () => {
  ...
  const createConfetti = () => {
    ...
    particlesRef.current = Array(150).fill().map(() => new Confetti())
  ...
  return (
    <main className="relative min-h-screen pt-24 pb-16 px-6 sm:px-12 md:px-24 max-w-4xl mx-auto">
      {/* Background Video */}
      <video 
      ...
      {!formSubmitted ? (
        ...
        >
          ...
          <fieldset className="mb-6">
            <legend className="block text-[#2F2F2F] font-semibold mb-2 text-lg">
              Pick a vibe:
            </legend>
            ...
            <input 
              type="hidden" 
              id="vibesInput" 
              value={selectedVibes.map(v => v.text).join(', ')} 
            />
          </fieldset>
        ...
        <div className="bg-white bg-opacity-90 backdrop-blur-md rounded-lg border border-[#d1a954] p-8 text-center max-w-2xl mx-auto">
          ...
          <div className="signature text-4xl font-bold text-[#D1A954] mt-4">
            Thank you
          </div>
        </div>
      )}
